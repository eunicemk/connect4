...
function Possible(token)
{
	...
	for(var x=0; x < 7; x++)
	{
		...
		for(var i = 1; i < 4; i++)
			{
				if(row+i < 6 && board[row+i][x] == token)
					count++;
				else
					break;
			}
		if(count == 3) //if the other player is going to win, we wish to block it
			return x;
		else //o.w. keep track of the connected number and the column
			{
				if(count > connectedNum)
				{
					connectedNum = count;
					colNum = x;
				}
		...
		for(var i = 1; i < 4; i++)
			{
				if(row-i>=0 && x+i>=0 && board[row-i][x+i] == token)
					count++;
				else
					break;
			}
		...
		for(var i = 1; i < 4; i++)
			{
				if(row+i<6 && x-i>=0 && board[row+i][x-i] == token)
					count++;
				else
					break;
			}
		...
		for(var i = 1; i < 4; i++)
			{
				if(row+i<6 && x+i<7 && board[row+i][x+i] == token)
					count++;
				else
					break;
			}
	...
	return [colNum,connectedNum]
}

function aiturn()
{
	var PCol1, PCol2;
	PCol1 = Possible(((currentPlayer+1)%2)); //check if the other player is going to win
	PCol2 = Possible((currentPlayer%2)); //check if AI has a chance to win
	console.log('Col1 =' + PCol1[0] + '  PC2 ='+PCol2[0]);
	if(PCol2[1] >= 2){
		if(PCol1[1] == 3){
			col(PCol1[0]);
		}
